...
def make_train(input_size,output_size,mem_size,mem_width,hidden_sizes=[100]):
    ...
    predict = model.build(P,mem_size,mem_width,hidden_sizes[-1],ctrl,no_heads=1)
    
    input_seq = T.matrix('input_sequence')
    output_seq = T.matrix('output_sequence')
    seqs = predict(input_seq)
    output_seq_pred = seqs[-1]
    cross_entropy = T.sum(T.nnet.binary_crossentropy(T.clip(output_seq_pred, 5e-6, (1 - 5e-6)),output_seq),axis=1)
...
if __name__ == "__main__":
    ...
    try:
        ...
        scores = []
        difficulty = 1
        #P.load('l2_repeat_256_2.mdl')
        persistence = 0
        cutoff = 0.01
        for counter in xrange(max_sequences):
            
            length = np.random.randint(difficulty) + 10
            if length > 30:
                length = 30
            num_repeats = np.random.randint(min(int(difficulty/3)+1, 20))+1    
            
            i,o = tasks.repeat_copy(9, length, num_repeats)
            if score == None: score = train(i,o)
            else: score = alpha * score + (1 - alpha) * train(i,o)
            print str(counter)+" "+str(score)+' '+str(length)+' '+str(num_repeats)+' '+str(difficulty)+' '+str(cutoff)
            scores.append(score)
           
            if score < cutoff * length*num_repeats:
                persistence += 1
            else:
                persistence = 0
            k = False
            if msvcrt.kbhit() :
                ...
                if inp == chr(13):
                    ...
                    cPickle.dump(scores, open('scores_repeat_256_2.pkl', 'wb'), cPickle.HIGHEST_PROTOCOL)
            if (score < cutoff * length*num_repeats and persistence > 30) or k:
                persistence = 0
                difficulty += 1
