...
if __name__ == "__main__":
    ...
    for counter in xrange(max_sequences):
        ...
        length = np.random.randint(int(counter/10000)+1) + 10 + np.random.randint(-2,3)
        num_repeats = np.random.randint(max(int(counter/10000), 3)+1) + 1        
        ...
        if score < best_score:
            ...
            P.save(model_out)
            best_score = score            
            cPickle.dump(scores, open('scores_repeat_2.pkl', 'wb'), cPickle.HIGHEST_PROTOCOL)
